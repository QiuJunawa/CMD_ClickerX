...
string chooser(string node, int long__, bool disable_ansi) {
    ...
    int clear_lines = 0;
    if (cc <= 2 * long__ + 1 || long__ < 0) {
        clear_lines = cc + 4; // 标题+选项+分隔线
    } else {
        clear_lines = 2 * long__ + 5; // 滚动列表显示行数
    }
...
int main() {
    ...
    #define DEBUG
    printf("连点器v4.1.3重置版--Build Succeed in : 03/07/25 12:06\n检查ANSI支持...");
    gotoxy(0, 2); 
    Loader loader;
    gotoxy(0, 1); 
    ...
    string menu_path[100];  // 存储导航路径的数组
    int path_index = 0;
